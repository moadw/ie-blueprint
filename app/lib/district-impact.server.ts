 ...
 *  - con data real no hay autor (solo `user` ID) ni nombre de escuela (solo
 *    `school` ID) → esos campos quedan null hasta resolverlos. Los samples sí los
 *    traen, así que la vista hoy se ve completa.
 */
...
export async function getDistrictImpact(request: Request): Promise<{
  ...
  if (impactResult.ok && impactResult.data.ImpactFindMany) {
    realStories = impactResult.data.ImpactFindMany.filter(Boolean).map((raw) => {
      ...
      return {
        ...
        authorName: null, // real data solo trae `user` (ID) — follow-up
        authorRole: null,
        schoolName: null, // real data solo trae `school` (ID) — follow-up
